...
def register():
    ...
    if register_form.validate_on_submit():
        existing_user = db.session.query(User).filter_by(email=register_form.email.data).first()
        if not existing_user:
            ...
            flash("That user already exists. Log in.")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if user:
            ...
            flash("That user does not exist. Try to register.")
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
    return render_template("post.html", post=requested_post, comment_form=comment_form, logged_in=current_user.is_authenticated)
